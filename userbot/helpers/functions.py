...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    speedo = await sticcers[0].click("me", hide_via=True)
    if speedo:
        await bot.send_file(int(chat_id), speedo, reply_to=reply_to_id)
        await speedo.delete()
