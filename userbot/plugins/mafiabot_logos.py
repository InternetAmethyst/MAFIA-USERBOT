...
async def _(speedoevent):
    event = await edit_or_reply(speedoevent, "`Processing.....`")
    fnt = await get_font_file(speedoevent.client, "@D3VIL_FONTSS")
    ...
    await bot.send_file(
        speedoevent.chat_id,
        file_name,
        caption=f"**мα∂ε вү  :** 『😈[{DEFAULTUSER}](tg://user?id={speedo})😈』\n\n",
    )
    await event.delete()
    try:
        os.remove(file_name)
