...
class speedoBroadcast(BASE):
    ...
    def __repr__(self):
        return "<speedo Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, speedoBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


speedoBroadcast.__table__.create(checkfirst=True)

speedoBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with speedoBROADCAST_INSERTION_LOCK:
        broadcast_group = speedoBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with speedoBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(speedoBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with speedoBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(speedoBroadcast.keywoard)
            .filter(speedoBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(speedoBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(speedoBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(speedoBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(speedoBroadcast).all()
        for x in all_groups:
